...
## Model: Booking (UPDATED — new statuses, product() method)
...
class Booking extends Model
{
    ...
    // CRITICAL: method is named 'product' NOT 'vendor_product'
    // Loading 'vendor_product' causes RelationNotFoundException
    public function product()
    {
        return $this->belongsTo(VendorProduct::class, 'vendor_product_id');
    }
...
Booking      → belongsTo → VendorProduct (via vendor_product_id) — method named 'product()'
...
**Problem:** `$booking->load(['vendor', 'vendor_product'])` throws RelationNotFoundException.
**Root Cause:** Relationship method in Booking model is named `product()` not `vendor_product()`.
**Fix:** Always use `$booking->load(['vendor', 'product'])`.
...
1. **Relationship Loading:** ALWAYS `$booking->load(['vendor', 'product'])` NEVER `vendor_product`
